...
def object_detection(coords):
    # Example implementation: Replace this with your actual object detection model
    # This is just a dummy implementation to provide random bounding boxes and confidence scores.
    num_boxes = np.random.randint(1, 5)
    # boxes = []
    # conf_scores = []

    x1 = np.random.randint(0, image.shape[1] - 50)
    y1 = np.random.randint(0, image.shape[0] - 50)
    x2 = np.random.randint(x1 + 10, image.shape[1])
    y2 = np.random.randint(y1 + 10, image.shape[0])
    # boxes.append([x1, y1, x2, y2])
    # conf_scores.append(np.random.rand())
    #
...
def NMS(boxes, conf_scores, overlapThresh=0.4):
    ...
    return boxes[indices].astype(int), conf_scores[indices]
...
def find_tfl_lights(c_image: np.ndarray, **kwargs) -> Tuple[
    ...
    for red_coords in zip(red_x, red_y):
        coords = object_detection(red_coords)
    ...
    NMS(boxes, conf_scores)
    # Example usage:
    # image = np.random.rand(100, 100)  # Replace this with your actual image
    # detected_boxes, confidence_scores = object_detection(red_x, red_y)
    # nms_boxes, nms_conf_scores = NMS(detected_boxes, confidence_scores)
    # print("Detected Boxes:\n", detected_boxes)
    # print("Confidence Scores:\n", confidence_scores)
    # print("NMS Result:\n", nms_boxes)
    # print("NMS Confidence Scores:\n", nms_conf_scores)
