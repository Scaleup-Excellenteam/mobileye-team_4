...
def crop_and_save_boxes(image: np.ndarray, x_coords_list, y_coords_list, save_directory, color):
    ...
    for idx, (x, y) in enumerate(zip(x_coords_list, y_coords_list)):
        ...
        crop_size = 100
        crop_width = 40
        x_max = min(image.shape[1], int(x + crop_width // 2))
        x_min = max(0, int(x - crop_size // 2))

        if color == 'red':
            y_min = max(0, int(y - 30))
            y_max = min(image.shape[0], int(y + 70))
        elif color == 'green':
            y_min = max(0, int(y - 50))
            y_max = min(image.shape[0], int(y + 20))
        else:
            raise ValueError("Invalid color. Color should be 'red' or 'green'.")
...
def test_find_tfl_lights(image_path: str, image_json_path: Optional[str] = None, fig_num=None):
    ...
    crop_and_save_boxes(c_image, red_x, red_y, save_directory, 'red')
